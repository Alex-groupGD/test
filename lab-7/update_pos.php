...
$postion = new select;
$postion = $postion->select_pos_where_id($_POST['id']);
...
<!--form----------------------------------------------------------------------------->
<?php foreach ($postion as $pos) ?>
<form  action="update_logic.php" method="post">


    <input class="mb-2   form-control me-2" type="hidden" name="id" value="<?=$pos['id']?>" >

    <div class="mb-3">
        <label for="formFile" class="form-label" ><dr>Изменить картинку:</label>
    ...
    <input class="mb-2   form-control me-2" type="number" name="price" value="<?=$pos['COAST']?>" >

    <p style="position: relative; text-align: center;"> Вид изделия:</p>

    <div class="" >
        <select name="customer_Customer" class="form-control"  >
            <option value="<?php  $pos['customer_id']?>" ><?= $pos['customer_Customer']?></option>
    ...
    <div class="input-group">
        ...
        <textarea class="form-control" name="TEXT_com" placeholder="Введите описание товара:"  ><?=$pos['TEXT_com']?></textarea>
    </div>

    <p style="position: relative; text-align: center;">Толщина изделия(в мм):</p>

    <input class="mb-2 form-control me-2" name="transactions_thickness" type="text" placeholder="Введите толщину товара:" value="<?=$pos['transactions_thickness']?>">
    <button  type="submit"  class="btn btn-primary mt-3 me-3 btn-sm">Изменить позицию</button>
</form>
